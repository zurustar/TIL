...
PostgreSQLで使用されるファイルの多くは「データベースクラスタ」と呼ばれるディレクトリ配下に作成される。
データベースクラスタはinitdbコマンド(もしくはpg_ctlコマンド)を用いることで実行したOSユーザのみアクセス可能な権限(700)で作成される。
権限が700になっていない場合はPostgreSQLの起動時にエラーとなる。

データベースクラスタ内の主なディレクトリを以下に示す。

| 名称 | 用途 |
| --- | --- |
| base | データベースごとに作成される識別子oid(Object ID)を示す数字が名称となっているディレクトリが格納されるディレクトリ |
| global | データベースクラスタで共有するテーブルを保有するディレクトリ。pg_databaseなどの複数のデータベースにまたがるシステムカタログなどが格納される |
| pg_wal | WALファイルを格納するディレクトリ。データベースクラスタ作成時のオプションによってはシンボリックリンクになる場合あり。 |
| pg_xact | トランザクションのコミット状態を管理するファイルが格納される |
| pg_tblspc | テーブル空間として作成されたディレクトリのシンボリックリンクを格納 |

主なファイル

| 名称 | 用途 |
| --- | --- |
| PG_VERSION | PostgreSQLのメジャーバージョンが書かれている |
| テーブルファイル | テーブルデータの実体。データベースディレクトリに格納される |
| インデックスファイル | インデックス情報。データベースディレクトリに格納される |
| TOASTファイル | テーブル内に長大な行を格納する場合に生成・使用される |
| FREE Space Map ファイル | テーブル・インデックスごとに作成される。ファイル名は「テーブルやインデックスを示す数字_fsm」 |
| Visibility Map ファイル | 「テーブルを示す数字_vm」 |
| postmater.pid | 二重起動を防止するロックファイル |
